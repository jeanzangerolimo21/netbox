...
# PROCESSING
# =========================================================
def process_vm(proxmox, px_conf, context, node, vm, vm_type, cluster_id, rack_name=None, site_name=None):
    ...
    client_info = context.csv_inventory.get(
        str(vmid).strip()
    )
    ...
    if not client_info:
        log(f"[CSV DEBUG] VMID {vmid} não encontrado no arquivo CSV. IDs disponíveis: {list(context.csv_inventory.keys())[:5]}")
